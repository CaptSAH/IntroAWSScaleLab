...
```
   ...
   :muscle: *BONUS challenge* - In real-life, the rest of the template would be useful for the scenario described, where URLs would need to be generated to access the web-facing S3 bucket content. Can you find the resulting output on the stack status screen? Hint:  It's similar to finding the stack's associated template.
